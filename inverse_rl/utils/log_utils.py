...
def load_experts(fname, max_files=float('inf'), min_return=None, visible_gpus=None):
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    if visible_gpus is not None:
        config.gpu_options.visible_device_list = visible_gpus
...
def load_latest_experts(logdir, n=5, min_return=None, visible_gpus=None):
    return load_experts(get_expert_fnames(logdir, n=n), min_return=min_return, visible_gpus=visible_gpus)


def load_latest_experts_multiple_runs(logdir, n=5, visible_gpus=None):
    paths = []
    for i, dirname in enumerate(os.listdir(logdir)):
        dirname = os.path.join(logdir, dirname)
        if os.path.isdir(dirname):
            print('Loading experts from %s' % dirname)
            paths.extend(load_latest_experts(dirname, n=n, visible_gpus=visible_gpus))
    return paths
