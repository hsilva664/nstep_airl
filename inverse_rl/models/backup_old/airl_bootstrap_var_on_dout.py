...
class AIRL_Bootstrap(SingleTimestepIRL):
    ...
    def _combine_predictions(self):
            ...
            #TEMP
            log_pq = [ [None for i in range(self.n_rew_funct * self.n_value_funct)] for j in range(self.max_nstep)] 
            single_output = [ [None for i in range(self.n_rew_funct * self.n_value_funct)] for j in range(self.max_nstep) ]
            ...
            for i in range(self.n_rew_funct):
                for j in range(self.n_value_funct):
                    for single_nsteps in range(1,self.max_nstep+1):
                        ...
                        ##TEMP
                        log_pq[single_nsteps - 1][i*self.n_value_funct + j] = tf.reduce_logsumexp([log_p_tau[single_nsteps - 1][i*self.n_value_funct + j], log_q_tau], axis=0)
                        single_output[single_nsteps - 1][i*self.n_value_funct + j] = tf.exp(log_p_tau[single_nsteps - 1][i*self.n_value_funct + j]-log_pq[single_nsteps - 1][i*self.n_value_funct + j])                        
            ...
            # weights = tf.concat(variance_list, axis=1)
            # weights = tf.nn.softmax(1./(weights + 1e-8), axis=1)

            # # weights = tf.constant( ([0.0]*(self.max_nstep-1)) + [1.0], dtype=tf.float32 )

            # log_p_tau = tf.reduce_sum(weights * tf.concat(mean_list, axis=1), axis=1, keepdims = True)
            ...
            #TEMP
            for i in range(self.max_nstep):
                variance_list[i] = tf.math.reduce_variance( tf.stack(single_output[i]), axis=0 )
                single_output[i] = tf.reduce_mean( tf.stack(single_output[i]), axis=0 )        
                log_pq[i] = tf.reduce_mean( tf.stack(log_pq[i]), axis=0 )
                log_p_tau[i] = tf.reduce_mean( tf.stack(log_p_tau[i]), axis=0 )                
            ...
            self.ensemble_discrim_output = tf.reduce_sum(self.weights * tf.concat(single_output, axis=1), axis=1, keepdims = True)
            log_pq = tf.reduce_sum(self.weights * tf.concat(log_pq, axis=1), axis=1, keepdims = True)
            log_p_tau = tf.reduce_sum(self.weights * tf.concat(log_p_tau, axis=1), axis=1, keepdims = True)
            self.ensemble_loss = -tf.reduce_mean(self.labels*(log_p_tau-log_pq) + (1-self.labels)*(log_q_tau-log_pq))     

            # self.ensemble_loss = -tf.reduce_mean(self.labels*(log_p_tau-log_pq) + (1-self.labels)*(log_q_tau-log_pq))     

            self.t0_reward = tf.reduce_mean(tf.concat(t0_reward, axis=1), keepdims=True)
            self.t0_value = tf.reduce_mean(tf.concat(t0_value, axis=1), keepdims=True)
    ...
    def fit(self, paths, policy=None, batch_size=32, logger=None, lr=1e-3,**kwargs):
        ...
        if logger:            
            ...
            logger.record_tabular('GCLAverageLogPtau', np.mean(-np.mean(energy, axis=1) - logZ))
